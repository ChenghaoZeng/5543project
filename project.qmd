...
```{r}
# Boxplot: Affordability ratio vs housing stress
ggplot(explore2019, aes(x = factor(HousingStress), y = Affordability_Ratio, fill = factor(HousingStress))) +
  geom_boxplot() +
  scale_x_discrete(labels = c("0" = "Not stressed", "1" = "Stressed")) +
  labs(
    title = "Affordability Ratio by Housing Stress",
    x = "Housing Stress",
...
explore2019 %>%
  group_by(HousingStress) %>%
  summarise(
    mean_afford = mean(Affordability_Ratio, na.rm = TRUE),
    median_afford = median(Affordability_Ratio, na.rm = TRUE),
    mean_child_age = mean(Age_Youngest_Child, na.rm = TRUE)
  )
#t-test (for 2 groups)
t.test(Affordability_Ratio ~ HousingStress, data = explore2019)

```


```{r}
# proportion bar chart: Household size vs housing stress
explore2019 %>%
  group_by(Household_Size, HousingStress) %>%
  summarise(n = n()) %>%
  mutate(prop = n / sum(n)) %>%
  ggplot(aes(x = factor(Household_Size), y = prop, fill = factor(HousingStress))) +
  geom_col(position = "fill") +
  labs(
    title = "Proportion of Households Experiencing Housing Stress by Household Size",
    x = "Household Size (number of people)",
    y = "Proportion",
    fill = "Housing Stress"
  ) +
  scale_fill_manual(values = c("#74add1", "#f46d43"),
                    labels = c("Not stressed", "Stressed")) +
  theme_minimal()


#t-test (for 2 groups)
t.test(Household_Size ~ HousingStress, data = explore2019)
```
```{r}
# Boxplot: Age of youngest child vs housing stress
ggplot(explore2019, aes(x = factor(HousingStress), y = Age_Youngest_Child, fill = factor(HousingStress))) +
  geom_boxplot() +
  labs(
    title = "Age of Youngest Dependent Child by Housing Stress",
    x = "Housing Stress",
    y = "Age of Youngest Child (years)"
  ) +
  theme_minimal()

#t-test (for 2 groups)
t.test(Age_Youngest_Child ~ HousingStress, data = explore2019)
```

```{r}
ggplot(explore2019, aes(x = factor(Education_Level), fill = factor(HousingStress))) +
  geom_bar(position = "fill") +
  labs(
    title = "Proportion of Households Experiencing Housing Stress by Education Level",
    x = "Education Level",
    y = "Proportion within Education Level",
    fill = "Housing Stress"
  ) +
  scale_fill_manual(
    values = c("#74add1", "#f46d43"),
    labels = c("0" = "Not stressed", "1" = "Stressed")
  ) +
  ...
  "5" = "Certificate III and IV",
  "6" = "Year 12 or equivalent",
  "7" = "Year 11",
  "8" = "Year 10",
  "9" = "Certificate I and II",
  "10" = "Year 9 and below",
  "11" = "Certificate n.f.d.",
  "12" = "Level not determined (non-school only)")) +
  theme_minimal() +
  theme(
...
#t-test (for 2 groups)
t.test( Education_Level ~ HousingStress, data = explore2019)
```

```{r}
ggplot(explore2019, aes(x = factor(Tenure_type), fill = factor(HousingStress))) +
  geom_bar(position = "fill") +
  labs(
    title = "Proportion of Households Experiencing Housing Stress by Tenure Type",
    x = "Tenure Type",
    y = "Proportion within Tenure Type",
    fill = "Housing Stress"
  ) +
  scale_fill_manual(
    values = c("#74add1", "#f46d43"),
...
#t-test (for 2 groups)
t.test( Tenure_type ~ HousingStress, data = explore2019)
```

```{r}
# Create a named vector for state labels
state_labels <- c(
  "1" = "New South Wales",
  "2" = "Victoria",
  "3" = "Queensland",
...
# Plot proportion of housing stress by state
ggplot(explore2019, aes(x = factor(State), fill = factor(HousingStress))) +
  geom_bar(position = "fill") +
  labs(
    title = "Proportion of Households Experiencing Housing Stress by State",
    x = "State",
...
#t-test (for 2 groups)
t.test( State ~ HousingStress, data = explore2019)
```

```{r}
explore2019 <- explore2019 %>%
  mutate(
...
model <- glm(HousingStress ~ Life_Cycle_Group + Household_Size + 
                Affordability_Ratio + Education_Level + 
                Age_Youngest_Child + State + Tenure_type,
              data = explore2019,
...
```{r}
# Interaction between Affordability_Ratio and Education_Level
...
```{r}
#test If the interaction isn’t significant
