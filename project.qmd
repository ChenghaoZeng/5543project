...
stress_by_sex <- hh_svy %>%
  mutate(sex_group = factor(SEXRH, levels = unique(SEXRH)),
...
p_sex <- ggplot(plot_sex, aes(x = reorder(sex_group, pct), y = pct, fill = sex_group)) +
  geom_col(width = 0.6, color = "white", show.legend = FALSE) +
  geom_errorbar(aes(ymin = low, ymax = up), width = 0.2, color = "black") +
  coord_flip() +
  scale_y_continuous(labels = function(x) paste0(x, "%"), expand = c(0, 0)) +
  scale_fill_manual(values = c("#4E79A7", "#F28E2B")) +  # blue & orange tones
  labs(
    title = "Proportion of Households in Housing Stress (>30% of Income)",
    subtitle = "By Sex Group (with 95% Confidence Intervals)",
    x = NULL,
    y = "Percent of households (weighted)",
