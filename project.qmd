...
na_counts <- explore2019 %>%
  summarise(
    across(everything(), ~ sum(is.na(.)))  # 对所有变量，计算NA的数量
  )
t(na_counts)
...
model <- glm(HousingStress ~ Life_Cycle_Group + Household_Size + 
                     ...
                     Purchase_price_dwelling + Affordability_Ratio + sex_group + hh_total_weekly_repayments,  
...
```{r}
model2 <- glm(HousingStress ~ Tenure_type + Affordability_Ratio+ Household_Size + Age_Youngest_Child + 
                     Education_Level + State + Life_Cycle_Group + 
                     Purchase_price_dwelling + sex_group + hh_total_weekly_repayments,  
                       data = explore2019_imputed,
                       family = binomial(link = "logit")) %>% 
  step(trace=FALSE)
summary(model2)
